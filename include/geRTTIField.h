...
namespace geEngineSDK {
  ...
  namespace RTTI_FIELD_FLAG {
    enum E {
      ...
      /**
       * @brief Lets the replication system know that this field should be monitored for
       *        changes and replicated across the network when changes are detected.
       */
      kReplicate = 1 << 2
    };
  }

  using RTTIFieldFlags = Flags<RTTI_FIELD_FLAG::E>;
  GE_FLAGS_OPERATORS(RTTI_FIELD_FLAG::E);
  ...
  struct GE_UTILITIES_EXPORT RTTIField
  {
    ...
    /**
     * @brief Returns additional information about the field.
     */
    const RTTIFieldInfo&
    getInfo() const {
      return m_info;
