    ...
    for (auto& entry : allTypes) {
      ...
      for (uint32 i = 0; i < myNumFields; ++i) {
        ...
        for (uint32 j = 0; j < otherNumFields; ++j) {
          ...
          auto otherReflectablePtrField =
            static_cast<RTTIReflectablePtrFieldBase*>(otherField);

          if (myType->getRTTIId() == otherReflectablePtrField->getType()->getRTTIId() &&
              (!myReflectablePtrField->getInfo().flags.isSet(RTTI_FIELD_FLAG::kWeakRef) &&
              !otherReflectablePtrField->getInfo().flags.isSet(RTTI_FIELD_FLAG::kWeakRef))) {
