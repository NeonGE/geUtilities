...
namespace geEngineSDK {
              ...
              case SERIALIZABLE_FIELD_TYPE::kReflectablePtr:
              {
                ...
                for (auto& arrayElem : arrayData->entries) {
                  ...
                  if (nullptr != childRtti) {
                    ...
                    bool needsDecoding = !curField->getInfo().flags.
                                          isSet(RTTI_FIELD_FLAG::kWeakRef) &&
                                         !objToDecode.isDecoded;
                    if (needsDecoding) {
                      if (objToDecode.decodeInProgress) {
                        GE_LOG(kWarning, Generic, "Detected a circular "
                               "reference when decoding. Referenced object "
                               "fields will be resolved in an undefined "
                               "order (i.e. one of the objects will not be "
                               "fully deserialized when assigned to its "
                               "field). Use RTTI_Flag_WeakRef to get rid of "
                               "this warning and tell the system which of the "
                               "objects is allowed to be deserialized after "
                               "it is assigned to its field.");
                      }
                      else {
                        objToDecode.decodeInProgress = true;
                        decodeEntry(objToDecode.object, objToDecode.serializedObject);
  ...
  SerializedDataBlock::clone(bool cloneData) {
    ...
    if (cloneData) {
      if (stream->isFile()) {
        GE_LOG(kWarning, Generic, "Cloning a file stream. Streaming is "
               "disabled and stream data will be loaded into memory.");
      }

      auto* data = reinterpret_cast<uint8*>(ge_alloc(size));
