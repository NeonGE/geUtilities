...
namespace geEngineSDK {
  ...
  ColorGradient::setKeys(const Vector<ColorGradientKey>& keys,
    ...
    if (keys.size() > MAX_KEYS) {
      GE_LOG(kWarning, Generic, "Number of keys in ColorGradient exceeds the "
             "supported number ({0}). Keys will be ignored.", MAX_KEYS);
    }
  ...
  ColorGradientKey
  ColorGradient::getKey(uint32 idx) const {
    if (idx >= m_numKeys) {
      return ColorGradientKey(LinearColor::Black, 0.0f);
    }

    return ColorGradientKey(m_colors[idx], m_times[idx]);
  }
  ...
  bool
  ColorGradient::operator==(const ColorGradient& rhs) const {
    if (m_numKeys != rhs.m_numKeys || m_duration != rhs.m_duration) {
      return false;
    }

    for (uint32 i = 0; i < m_numKeys; ++i) {
      if (m_colors[i] != rhs.m_colors[i] || m_times[i] != rhs.m_times[i]) {
        return false;
      }
    }
